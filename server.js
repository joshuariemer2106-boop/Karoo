  ...
  readBody(req)
    .then(raw => {
      ...
      if (!name || !email || !password) {
        sendJson(res, 400, { error: "Bitte alle Felder ausfuellen." });
        return;
      }

      if (password.length < 6) {
        sendJson(res, 400, { error: "Passwort muss mindestens 6 Zeichen haben." });
        return;
      }

      if (findUserByEmail(email)) {
        sendJson(res, 400, { error: "E-Mail bereits registriert." });
        return;
      }
...
function handleLogin(req, res) {
  readBody(req)
    .then(raw => {
      ...
      if (!email || !password) {
        sendJson(res, 400, { error: "Bitte E-Mail und Passwort eingeben." });
        return;
      }

      const user = findUserByEmail(email);
      if (!user || user.password !== password) {
        sendJson(res, 401, { error: "Ungueltige E-Mail oder Passwort." });
        return;
      }
...
function handleLogout(req, res) {
  readBody(req)
    .then(raw => {
      ...
      sendJson(res, 200, { message: "Erfolgreich abgemeldet." });
    })
    .catch(error => {
      sendJson(res, 500, { error: error.message });
    });
...
function handleGetUser(req, res) {
  ...
  if (!session) {
    sendJson(res, 401, { error: "Session abgelaufen." });
    return;
  }
